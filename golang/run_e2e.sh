#!/usr/bin/env bash
#
# BleepStore Go — E2E Test Runner
#
# Builds the project, starts the server, runs E2E tests, stops the server.
# Logs are written to golang/logs/ (gitignored).
#
# Usage:
...
echo "Starting BleepStore Go on port $PORT..."
./bleepstore --config "$CONFIG_FILE" --port $PORT \
    > "$SERVER_LOG" 2>&1 &
SERVER_PID=$!
