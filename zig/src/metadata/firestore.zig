...
pub const FirestoreConfig = struct {
    collection: []const u8,
    project: []const u8,
...
pub const FirestoreMetadataStore = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, config: FirestoreConfig) !Self {
        const http_client = std.http.Client{ .allocator = allocator };
        const access_token = try allocator.dupe(u8, config.access_token);

        std.log.info("Firestore metadata store initialized: collection={s} project={s}", .{ config.collection, config.project });
    ...
    fn makeRequest(self: *Self, method: std.http.Method, path: []const u8, body: ?[]const u8) ![]u8 {
        const url = try std.fmt.allocPrint(self.allocator, "https://firestore.googleapis.com/v1/{s}", .{path});
        ...
        const result = self.http_client.fetch(.{
            ...
            std.log.err("Firestore request error: {}", .{err});
            return err;
        };

        if (@intFromEnum(result.status) >= 400) {
            std.log.err("Firestore error: status={d} body={s}", .{ @intFromEnum(result.status), response_body_list.items });
        }
    ...
    fn createBucket(ctx: *anyopaque, meta: BucketMeta) anyerror!void {
        ...
        const path = try std.fmt.allocPrint(self.allocator, "projects/{s}/databases/(default)/documents/{s}/{s}", .{ self.config.project, self.config.collection, doc_id });
        defer self.allocator.free(path);

        const body = try std.fmt.allocPrint(self.allocator,
            \\{{"fields":{{"type":{{"stringValue":"bucket"}},"name":{{"stringValue":"{s}"}},"region":{{"stringValue":"{s}"}},"owner_id":{{"stringValue":"{s}"}},"owner_display":{{"stringValue":"{s}"}},"acl":{{"stringValue":"{s}"}},"created_at":{{"stringValue":"{s}"}}}}}}
        , .{ meta.name, meta.region, meta.owner_id, meta.owner_display, meta.acl, nowIso() });
        defer self.allocator.free(body);

        const resp = try self.makeRequest(.PATCH, path, body);
    ...
    fn updateBucketAcl(ctx: *anyopaque, name: []const u8, acl: []const u8) anyerror!void {
        ...
        const path = try std.fmt.allocPrint(self.allocator, "projects/{s}/databases/(default)/documents/{s}/{s}?updateMask.fieldPaths=acl", .{ self.config.project, self.config.collection, doc_id });
        defer self.allocator.free(path);

        const body = try std.fmt.allocPrint(self.allocator, "{{\"fields\":{{\"acl\":{{\"stringValue\":\"{s}\"}}}}}}", .{acl});
        defer self.allocator.free(body);
    ...
    fn putObjectMeta(ctx: *anyopaque, meta: ObjectMeta) anyerror!void {
        ...
        const body = try std.fmt.allocPrint(self.allocator,
            \\{{"fields":{{"type":{{"stringValue":"object"}},"bucket":{{"stringValue":"{s}"}},"key":{{"stringValue":"{s}"}},"size":{{"integerValue":"{d}"}},"etag":{{"stringValue":"{s}"}},"content_type":{{"stringValue":"{s}"}},"storage_class":{{"stringValue":"{s}"}},"acl":{{"stringValue":"{s}"}},"last_modified":{{"stringValue":"{s}"}}}}}}
        , .{ meta.bucket, meta.key, meta.size, meta.etag, meta.content_type, meta.storage_class, meta.acl, nowIso() });
        defer self.allocator.free(body);
    ...
    fn getObjectMeta(ctx: *anyopaque, bucket: []const u8, key: []const u8) anyerror!?ObjectMeta {
        ...
        return ObjectMeta{
            ...
            .size = try self.extractInt(resp, "size") orelse 0,
    ...
    fn deleteObjectMeta(ctx: *anyopaque, bucket: []const u8, key: []const u8) anyerror!bool {
        ...
        return std.mem.indexOf(u8, resp, "\"error\":") == null;
    }
    ...
    fn createMultipartUpload(ctx: *anyopaque, meta: MultipartUploadMeta) anyerror!void {
        ...
        const body = try std.fmt.allocPrint(self.allocator,
            \\{{"fields":{{"type":{{"stringValue":"upload"}},"upload_id":{{"stringValue":"{s}"}},"bucket":{{"stringValue":"{s}"}},"key":{{"stringValue":"{s}"}},"content_type":{{"stringValue":"{s}"}},"storage_class":{{"stringValue":"{s}"}},"acl":{{"stringValue":"{s}"}},"initiated_at":{{"stringValue":"{s}"}}}}}}
        , .{ meta.upload_id, meta.bucket, meta.key, meta.content_type, meta.storage_class, meta.acl, nowIso() });
        defer self.allocator.free(body);
    ...
    fn putPartMeta(ctx: *anyopaque, upload_id: []const u8, part: PartMeta) anyerror!void {
        ...
        var doc_id_buf: [256]u8 = undefined;
        const upload_doc = std.fmt.bufPrint(&doc_id_buf, "upload_{s}", .{upload_id}) catch &doc_id_buf;
        var part_doc_buf: [32]u8 = undefined;
        const part_doc = std.fmt.bufPrint(&part_doc_buf, "part_{d:0>5}", .{part.part_number}) catch &part_doc_buf;

        const path = try std.fmt.allocPrint(self.allocator, "projects/{s}/databases/(default)/documents/{s}/{s}/parts/{s}", .{ self.config.project, self.config.collection, upload_doc, part_doc });
        defer self.allocator.free(path);

        const body = try std.fmt.allocPrint(self.allocator,
            \\{{"fields":{{"type":{{"stringValue":"part"}},"part_number":{{"integerValue":"{d}"}},"size":{{"integerValue":"{d}"}},"etag":{{"stringValue":"{s}"}},"last_modified":{{"stringValue":"{s}"}}}}}}
        , .{ part.part_number, part.size, part.etag, part.last_modified });
        defer self.allocator.free(body);
    ...
    fn listPartsMeta(ctx: *anyopaque, upload_id: []const u8, max_parts: u32, part_marker: u32) anyerror!ListPartsResult {
        ...
        const path = try std.fmt.allocPrint(self.allocator, "projects/{s}/databases/(default)/documents/{s}/{s}/parts", .{ self.config.project, self.config.collection, upload_doc });
        defer self.allocator.free(path);
    ...
    fn listMultipartUploads(ctx: *anyopaque, bucket: []const u8, prefix: []const u8, max_uploads: u32) anyerror!ListUploadsResult {
        ...
        const path = try std.fmt.allocPrint(self.allocator, "projects/{s}/databases/(default)/documents/{s}", .{ self.config.project, self.config.collection });
        defer self.allocator.free(path);
    ...
    fn putCredential(ctx: *anyopaque, cred: Credential) anyerror!void {
        ...
        const body = try std.fmt.allocPrint(self.allocator,
            \\{{"fields":{{"type":{{"stringValue":"credential"}},"access_key_id":{{"stringValue":"{s}"}},"secret_key":{{"stringValue":"{s}"}},"owner_id":{{"stringValue":"{s}"}},"display_name":{{"stringValue":"{s}"}},"active":{{"booleanValue":{s}}},"created_at":{{"stringValue":"{s}"}}}}}}
        , .{ cred.access_key_id, cred.secret_key, cred.owner_id, cred.display_name, if (cred.active) "true" else "false", nowIso() });
        defer self.allocator.free(body);
    ...
    fn extractField(self: *Self, json: []const u8, key: []const u8) !?[]const u8 {
        const search = try std.fmt.allocPrint(self.allocator, "\"{s}\":{{\"stringValue\":\"", .{key});
    ...
    fn extractInt(self: *Self, json: []const u8, key: []const u8) !?u64 {
        const search = try std.fmt.allocPrint(self.allocator, "\"{s}\":{{\"integerValue\":\"", .{key});
        defer self.allocator.free(search);
    ...
    fn extractBool(self: *Self, json: []const u8, key: []const u8) !?bool {
        const search = try std.fmt.allocPrint(self.allocator, "\"{s}\":{{\"booleanValue\":", .{key});
        defer self.allocator.free(search);
...
test "FirestoreMetadataStore: docIdBucket" {
...
test "FirestoreMetadataStore: docIdObject" {
    var buf: [1024]u8 = undefined;
    const result = std.fmt.bufPrint(&buf, "object_{s}_{s}", .{ "my-bucket", "my/key.txt" }) catch &buf;
    try std.testing.expectEqualStrings("object_my-bucket_my/key.txt", result);
}
